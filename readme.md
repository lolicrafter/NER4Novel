...
新增了基于规则和关键词的人物关系抽取功能，无需训练模型即可使用。支持以下关系类型：

- **家庭关系**：父子、母子、夫妻、兄弟、姐妹、祖孙等
- **社会关系**：朋友、师生、同学、同事、上下级等
- **情感关系**：恋人、仇敌、恩人等
...
# 启用调试模式，查看详细的关系抽取结果
python rel.py --book book_name --debug

# 禁用关系抽取（仅使用共现统计）
python rel.py --book book_name --no-relation

# 调整语义关系权重（0-1，默认0.3）
python rel.py --book book_name --relation-weight 0.5
```

### 工作原理

1. **共现统计**：原有的统计方法，计算人物在同一句话/段落中的共现次数
2. **语义关系抽取**：基于关键词和规则模式，识别明确的语义关系
3. **矩阵融合**：将两种方法的结果合并，得到更准确的关系矩阵

### 关系类型示例

支持的关系类型包括：
- 家庭关系：父子、母子、夫妻、兄弟、姐妹、祖孙、叔侄等
- 社会关系：朋友、师生、同学、同事、上下级、主仆等
- 情感关系：恋人、仇敌、恩人等
